...
// The scratch roots an agent can reach. /private/tmp is macOS-only — there /tmp is a
// symlink to it — while on Linux only /tmp exists. Both are scanned and a missing
// root costs nothing, so this list is portable as written.
export const SCAN_ROOTS = ["/tmp", "/private/tmp"];

// Where a test should plant a file so the guard will actually find it. Exported so a
// selftest cannot hardcode a path that exists on one platform and not the other —
// which is exactly how this first shipped: green on macOS, red on all four Linux
// cells, and the matrix is the only reason that was caught before a release.
...
export function assertNoStaleAnswers({ answerNames = [], contentKeys = [], runsRoot }) {
  const roots = [...SCAN_ROOTS, process.env.TMPDIR || "/tmp"];
  const found = [];
...
export function selftest() {
  ...
  check("assertNoStaleAnswers catches an answer file by name", () => {
    const scratch = mkdtempSync(join(SCRATCH, "bbstale-"));
