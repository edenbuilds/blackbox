...
const DEFAULTS = {
  ...
  reps: 1,
  timeoutSec: 900,
  maxCostUsd: null,      // stop the suite when cumulative spend reaches this
  maxWallClockS: null,   // stop the suite when it has run this long
};
...
export function runSuite({ spec, filter = {}, onLog = () => {} }) {
  ...
  // The budget is checked BETWEEN cells, not inside one. A cell is the smallest unit
  // that produces a scoreable result, so killing one mid-flight spends the money and
  // buys nothing — the cap exists to stop the next cell, not to salvage this one.
  const started = Date.now();
  let spent = 0;
  const budgetBreach = () => {
    if (spec.maxCostUsd != null && spent >= spec.maxCostUsd) {
      return `cost cap reached: $${spent.toFixed(4)} of $${spec.maxCostUsd}`;
    }
    if (spec.maxWallClockS != null) {
      const elapsed = Math.round((Date.now() - started) / 1000);
      if (elapsed >= spec.maxWallClockS) return `wall-clock cap reached: ${elapsed}s of ${spec.maxWallClockS}s`;
    }
    return null;
  };
  ...
  for (const task of spec.tasks) {
    ...
    for (const arm of Object.keys(spec.arms)) {
      ...
      for (let rep = 1; rep <= spec.reps; rep++) {
        ...
        if (breach) {
          // Recorded, not silently dropped. A suite that stopped early and a suite
          // that ran everything must not produce the same-looking scorecard.
          results.push({
            task: task.id, arm, rep, ran: false, passed: false, void: false,
            skipped: breach, ts: new Date().toISOString(),
          });
          continue;
        }
        try {
          const cell = runCell({ spec, task, arm, rep, onLog });
          if (cell.cost_usd != null) spent += cell.cost_usd;
          results.push(cell);
        } catch (e) {
...
export function score(results) {
  ...
  for (const r of results) {
    const a = (arms[r.arm] ??= {
      cells: 0, ran: 0, passed: 0, void: 0, blocked: 0, dead: 0, skipped: 0,
  ...
  for (const a of Object.values(arms)) {
    ...
    const scored = a.cells - a.void - a.blocked - a.dead - a.skipped;
    a.scored = scored;
...
export function formatScore(arms) {
  ...
  // passed/scored as one fraction, not two adjacent counts. Side by side they read as
  // two pass numbers — a real reader took "3  0" for three passes and no failures,
  // which inverts the finding. A table that can be read backwards is a bug.
  const lines = [
    `${"arm".padEnd(w)}  passed   rate    void  dead  blocked  cost      wall`,
  ];
  for (const [name, a] of Object.entries(arms)) {
    lines.push(
      `${name.padEnd(w)}  ${`${a.passed}/${a.scored}`.padStart(6)}  ` +
  ...
  if (sum("skipped")) {
    lines.push("");
    lines.push(`${sum("skipped")} cell(s) never started — the suite hit its budget cap and stopped.`);
    lines.push("The rates above cover only the cells that ran. This is a partial suite.");
  }
  if (sum("dead")) {
    lines.push("");
